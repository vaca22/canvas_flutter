...
class CheckmeProFile {


  final Uint8List originalData;
   List<double> waveData=[];

  CheckmeProFile({required this.originalData});
  ...
  void uncompress() {
    print(originalData);
    int dataLength=originalData[2] + (originalData[3] << 8) + (originalData[4] << 16) + (originalData[5] << 24);
    ...
    for(int i=0;i<dataLength-4;i+=2){
      ...
      int temp2=((originalData[dataPos+i+2]) + (originalData[dataPos+i+3]<< 8)).toSigned(16);
