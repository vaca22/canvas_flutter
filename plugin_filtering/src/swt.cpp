...
void SWT::CutSwd(int sampleCnt) {
    ...
    for (i = 0; i < SWT_FLOOR_NUM; i++) {
        ...
        sort(arrayForOrder.begin(),
             arrayForOrder.end());        // Panjie��sort���������ֱ��Ӧ������������׵�ַ��β��ַ

        if (sampleCnt % 2 == 0) {
            lamda = (arrayForOrder[sampleCnt / 2 - 1] + arrayForOrder[sampleCnt / 2]) /
                    2;        // Panjie: ȡ��ֵ
        } else {
        ...
        for (j = 0; j < sampleCnt; j++) {
            absX = fabs(swd[i][j]);
            if (-1 != DoubleCompares(absX,
                                     lamda))        // Panjie: lamdaΪ��ֵ����absX���ڵ���lamdaʱ����absX
            {
                continue;
            } else {
                swd[i][j] = 0;
